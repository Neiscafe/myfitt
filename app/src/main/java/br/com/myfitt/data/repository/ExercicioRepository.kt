...
class ExercicioRepository(private val dao: ExercicioDao) {
    suspend fun getExercicioPorNome(nome: String): Exercicio? = withContext(Dispatchers.IO) {
        return@withContext dao.getExercicioPorNome(nome)
    }

    suspend fun insertExercicio(exercicio: Exercicio): ExercicioEntity = withContext(
        Dispatchers.IO
    ) {
        val entity = exercicio.toEntity()
        return@withContext entity.copy(id = dao.insert(entity).toInt())
    }
